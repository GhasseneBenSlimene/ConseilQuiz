...
class CompanyListScreen extends StatefulWidget {
  ...
  const CompanyListScreen({Key? key, required this.userAnswers})
      : super(key: key);
  ...
  Future<void> loadRecommendations() async {
    try {
      final sortedCompanies =
          await _companyService.getSortedCompanies(widget.userAnswers);
      ...
      if (user != null) {
        ...
        setState(() {
          ...
          wishlist = wishlistDoc.exists
              ? List<String>.from(wishlistDoc['companyIds'] ?? [])
              : [];
          isLoading = false;
        });
      }
  ...
  Future<void> toggleWishlist(String companyId) async {
    ...
    setState(() {
      if (wishlist.contains(companyId)) {
        ...
        final company = companies.firstWhere((c) => c['company'] == companyId,
            orElse: () => {});
            ...
            content: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  _buildDetailRow(
                      "Date de publication", company['date']?.split('T')[0]),
                  _buildDetailRow(
                      "Niveau d'expérience requis", company['company_xp']),
                  _buildDetailRow(
                      "Expérience totale recommandée", company['total_xp']),
                  ...
                  child: ListTile(
                    ...
                    subtitle: Column(
                      ...
                      children: [
                        ...
                        Text(
                            "Expérience requise : ${company['company_xp'] ?? 'N/A'} ans"),
                      ],
                    ),
                    trailing: Row(
